...
import { LanguagesIntroScene } from "./scenes/LanguagesIntroScene";
...
const SCENES: SceneDef[] = [
  { id: "opening", duration: 2200, Component: OpeningScene },
  ...
  { id: "languages-intro", duration: 3600, Component: LanguagesIntroScene },
  { id: "constellation", duration: 6500, Component: ConstellationScene },
  { id: "collaboration", duration: 4200, Component: CollaborationScene },
  { id: "archetype", duration: 4800, Component: ArchetypeRevealScene },
  // ShareCardScene is the last scene and never auto-advances away from itself
...
export function StoryPlayer({ user, contributions, languages, onExit }: Props) {
  const reducedMotion = usePrefersReducedMotion();
  const insights = useMemo(
    () => computeInsights({ user, contributions, languages }),
    [user, contributions, languages]
  );
  const palette = usePalette(insights.primaryLanguage);

  const ctx: SceneContext = useMemo(
    () => ({ user, contributions, languages, insights, palette, reducedMotion }),
    [user, contributions, languages, insights, palette, reducedMotion]
  );
  ...
  const { index, progress, paused, next, prev, setPaused } = useStoryControls(
    ...
    durations,
    () => {
      /* on complete — stay on final scene */
    }
  );
